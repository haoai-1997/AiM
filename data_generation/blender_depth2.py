...
# ---------------- Paths ----------------
OBJ_ROOT = Path("/rds/projects/o/ofeke-dynamic-scene-reconstruction/haoai/data_generation/obj_file_200_open/artgs_multi")
IMG_ROOT = Path("/rds/projects/o/ofeke-dynamic-scene-reconstruction/haoai/data_generation/render_200_OPEN/artgs_multi")
...
def setup_scene_and_compositor(dst_dir: Path, res_x=800, res_y=800, engine='BLENDER_EEVEE_NEXT', clip_end_val=10000.0):
    ...
    fmt.exr_codec = 'ZIP'  
    try:
        n_file_exr.file_slots[0].use_node_path = False 
...
def main():
    ...
    meta = {"clip_start": float(cam_data.clip_start), "clip_end": float(cam_data.clip_end),
            "width": int(scene.render.resolution_x), "height": int(scene.render.resolution_y)}
    with open(dst_dir / "render_meta.json", "w") as f:
