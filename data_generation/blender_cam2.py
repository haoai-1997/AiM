...
def setup_lighting_and_world(radius):
    ...
    # === clean the current existing lights ===
    for L in [o for o in bpy.data.objects if o.type == 'LIGHT']:
        bpy.data.objects.remove(L, do_unlink=True)

    # === World：uniform "sky/ceil" environment light ===
    world = bpy.context.scene.world
    world.use_nodes = True
    nt = world.node_tree
    nodes, links = nt.nodes, nt.links

    # clean and reconstruct the world nodes
    for n in list(nodes):
        nodes.remove(n)
    ...
    # pure color and a little gray, avoiding over-exposure
    bg.inputs["Color"].default_value = (0.2, 0.2, 0.2, 1.0)
    bg.inputs["Strength"].default_value = 0.9 # environment magnitude 
    ...
    # normalization and light position
...
def camera_setting(exp_dir, n_frames=100, stage='train', track_to='start', radius=6.0, frame_id=0, category=None):
    ...
    if stage == 'test':  # turntable
        ...
        if frame_id == 0:
            ...
            # different scanning trajectory due to different object size
            if category in ["fridge"]:
                x = np.linspace(0, 10 * np.pi, 198)
                theta =  [-0.5 * np.pi - 0.5 * np.pi * np.sin(x[frame_id-1])]
                phi_min = 0.25 * np.pi
                phi_max = 0.5 * np.pi
                L = 35
                u = ((frame_id-1) % L) / L  # normalize the time [0,1)
                tri = 0.5 * (1 - np.cos(2 * np.pi * u)) 
                phi = [phi_min + (phi_max - phi_min) * tri]
            if category in ["table"]:
                x = np.linspace(0, 10 * np.pi, 198)
                theta =  [-0.5 * np.pi - 0.45 * np.pi * np.sin(x[frame_id-1])]
                phi_min = 0.25 * np.pi
                phi_max = 0.5 * np.pi
                L = 35
                u = ((frame_id-1) % L) / L  
            ...
            if category in ["oven"]:
                ...
                tri = 0.5 * (1 - np.cos(2 * np.pi * u))
                phi = [phi_min + (phi_max - phi_min) * tri]
            if category in ["foldchair"]:
                x = np.linspace(0, 10 * np.pi, 198)
                theta =  [-0.5 * np.pi - 0.7 * np.pi * np.sin(x[frame_id-1])]
                phi_min = 0.1 * np.pi
                phi_max = 0.5 * np.pi
                L = 35
                u = ((frame_id-1) % L) / L 
            ...
            if category in ["scissor"]:
                ...
                L = 35
                u = ((frame_id-1) % L) / L  
                tri = 1.0 - np.abs(2 * u - 1.0)  
                phi = [phi_min + (phi_max - phi_min) * tri]
            if category in ["stapler"]:
                x = np.linspace(0, 10 * np.pi, 198)
                theta = [-0.5 * np.pi - 1 * np.pi * np.sin(x[frame_id - 1])]
                phi_min = 0.1 * np.pi
                phi_max = 0.5 * np.pi
            ...
            if category in ["USB"]:
                x = np.linspace(0, 10 * np.pi, 198)
                theta = [0 * np.pi - 1 * np.pi * np.sin(x[frame_id - 1])]
                phi_min = 0.0 * np.pi
                phi_max = 0.5 * np.pi
                L = 35
                u = ((frame_id-1) % L) / L  
            ...
            if category in ["laptop"]:
                x = np.linspace(0, 10 * np.pi, 198)
                theta =  [-0.5 * np.pi - 0.65 * np.pi * np.sin(x[frame_id-1])]
                phi_min = 0.0 * np.pi
                phi_max = 0.45 * np.pi
                L = 35
                u = ((frame_id-1) % L) / L  
    ...
    if cam_obj:
        ...
        if track_to_constraint:
            print("camera-tracking target position:", track_to_constraint.target.location)
            if track_to_constraint.target.location == mathutils.Vector((0, 0, 0)):
                print("Tracking aligned")
            else:
                print("Tracking misaligned")
        else:
            print("No TRACK_TO constraint")
    else:
...
if __name__ == "__main__":
    # 例如：["/bask/projects/j/jiaoj-generative/haoai/PartNet-Mobility/Sapien/single-part/multi_frame/xxx"]
    category_name = sys.argv[-3].split("/")[0]
    category_id = sys.argv[-3].split("/")[1]

    objs = ["/rds/projects/o/ofeke-dynamic-scene-reconstruction/haoai/data_generation/obj_file_200_open/artgs_multi/" +
    ...
    for obj in objs:
        ...
        dst_dir = "/rds/projects/o/ofeke-dynamic-scene-reconstruction/haoai/data_generation/render_200_OPEN/artgs_multi/" + f_name + "/" + str(
            frame_id)
