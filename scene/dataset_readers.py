...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, state):
    cam_infos = []
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height, fid=fid, depth=depth)
...
def fetchPly(path):
    ...
    # np.vstack([vertices['red'], vertices['green'],
...
def readColmapSceneInfo(path, images, eval, llffhold=8, state= None):
    try:
    ...
    for k in list(cam_extrinsics.keys()):
        if (int(cam_extrinsics[k].name.split(".")[0])> 406 or int(cam_extrinsics[k].name.split(".")[0]) in
                [149, 236, 240, 304, 347, 403]):
            del cam_extrinsics[k]
    if state== "start":
        for k in list(cam_extrinsics.keys()):
            if int(cam_extrinsics[k].name.split(".")[0]) > 280:
                del cam_extrinsics[k]
    elif state == "motion":
        for k in list(cam_extrinsics.keys()):
            if  int(cam_extrinsics[k].name.split(".")[0]) < 280 or int(cam_extrinsics[k].name.split(".")[0]) > 362:
                del cam_extrinsics[k]
    elif state == "end":
        for k in list(cam_extrinsics.keys()):
            if  int(cam_extrinsics[k].name.split(".")[0]) < 362 or 372<int(cam_extrinsics[k].name.split(".")[0])< 395:
                del cam_extrinsics[k]
    else:
        pass
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics,
                                           images_folder=os.path.join(path, reading_dir), state = state)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", no_bg=True, load_depth=True):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                                        ...
                                        height=image.size[1], fid=frame_time, depth=depth))

    return cam_infos


def readNerfSyntheticInfo(path, white_background, eval, extension=".png", init_point_size = "large"):
