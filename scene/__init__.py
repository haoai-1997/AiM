...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True,
                 resolution_scales=[1.0], state="static", stage = None, init_point_size = "large"):
        ...
        if load_iteration:
            if load_iteration == -1:
                self.loaded_iter = searchForMaxIteration(os.path.join(self.model_path, f"point_cloud_{state}"))
            else:
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, state=state)

        elif os.path.exists(os.path.join(args.source_path, state, "transforms_train.json")):
            print("Found transforms_train.json file, assuming Blender data set!")
            scene_info = sceneLoadTypeCallbacks["Blender"](os.path.join(args.source_path, state), args.white_background,
                                                           args.eval, init_point_size = init_point_size)
        ...
        if os.path.exists(scene_info.ply_path):
            os.remove(scene_info.ply_path)
            print(f"Removing Initial Points in {scene_info.ply_path}")
        if shuffle:
    ...
    def save(self, iteration):
        point_cloud_path = os.path.join(self.model_path, f"point_cloud_{self.state}"+"/iteration_{}".format(iteration))
